...
router.post('/'  , [
    ...
    body('skills').notEmpty().withMessage('Skills Required'),
    body('bio').notEmpty().withMessage('Bio Required'),
    body('githubUsername').notEmpty().withMessage('githubUsername Required'),
    body('youtube').notEmpty().withMessage('Youtube Required'),
    body('facebook').notEmpty().withMessage('facebook Required'),
    body('twitter').notEmpty().withMessage('Twitter Required'),
    body('linkedin').notEmpty().withMessage('Linkedin Required'),
    body('instagram').notEmpty().withMessage('Instagram Required')
...
router.put('/'  , [
    ...
    try {
        ...
        if (!profile){
            return response.status(401).json({errors : [{msg : 'Profile not Exist'}]});
        }
        let profileObj = {};
        profileObj.user = request.user.id; // id gets from Token
        if (company) profileObj.company  = company;
...
router.get('/users/:user Id' ,async (request , response)=>{
    try {
        ...
        if (!profile){
            return response.status(401).json({errors : [{msg : 'Profile Not Found for the user'}]})
        }
        response.status(200).json({
            profile : profile
        })
